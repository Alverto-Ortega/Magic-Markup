...
function collectData() {
  chrome.storage.local.get(["daColor","daRange","daStop"], function (items){
    //console.log(items.daColor, items.daRange,items.daStop);  show data
      ...
      function draw(context, x1, y1, x2, y2) {
        ...
        context.beginPath();
        context.strokeStyle  = items.daColor; 
        context.lineWidth = items.daRange;
        context.shadowColor = items.daColor;
      ...
      chrome.runtime.onMessage.addListener(function(request, sender, sendRespose){
        ...
        chrome.storage.local.get(["daClear"], function (items){
          clearCanvas();
        });
      };
      });
